...
rule unicycler:
    ...
    shell:
        manager.get_shell("unicycler/run", "v1")
...
rule flye:
    ...
    shell:
        ...
        flye --genome-size {params.genome_size} {params.options} --{params.preset} {input} --out-dir ${{outdir}} --threads {threads} >{log} 2>&1    && mv ${{outdir}}/assembly.fasta {output.contig}
        """
